...
- npm パッケージ名は `genba-trace` です。画面上の名前は「参画前に知っておきたい Java Web アプリの読み解き方」です
...
| --- | --- |
| はじめに | この教材について |
| Web の基礎 | URL、ステータスコード、Cookie など HTTP の読み方に加え、フロントエンドとバックエンドの役割分担や Ajax・JSON の扱いも身につけます |
| Java アプリの構成 | リポジトリを開いたとき、ファイルと層の役割に加え、設定ファイルやトランザクション、共通処理の仕組みまで見渡せるようにします |
