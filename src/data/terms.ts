...
export const terms: TermDef[] = [
  ...
  },
  {
    ...
    body: "JVM がメモリを確保できなくなったときの例外です。`Java heap space` はヒープ不足、`Metaspace` はクラス情報の領域不足など、メッセージによって疑う場所が変わります。",
  },
  {
    term: "スレッドダンプ",
  ...
  },
  {
    ...
    body: "DB 側で実際に行を専有せず、更新時に条件（状態やバージョン番号）をチェックして、他の処理が先に進んでいないかを確認する方法です。実際にはロックを取っていません。",
  },
  {
    term: "悲観ロック",
    aliases: ["悲観ロック", "悲観的ロック"],
    body: "`SELECT ... FOR UPDATE` などで、読む時点から実際に行をロックする方法です。あとから来たトランザクションは、先のトランザクションが確定するまで待たされます。",
  },
  {
    term: "デッドロック",
    aliases: ["デッドロック"],
