...
export const terms: TermDef[] = [
  ...
  },
  {
    term: "GET / POST",
    aliases: ["GET", "POST"],
    body: "HTTP メソッドの代表例。GET は取得、POST は登録や状態を変える操作に使われることが多いです。PUT / PATCH / DELETE も API でよく使います。約束事と実装がずれることもあります。",
  },
  {
    term: "HTTPステータスコード",
  ...
  },
  {
    term: "401 / 403",
    aliases: ["401", "403"],
    body: "ステータスコードとしては、401 を未ログイン、403 を権限が無いと読みます。ただしこの 2 つが実際に返るとは限らず、遷移先やエラー画面もアプリ次第です。",
  },
  {
    term: "400",
    aliases: ["400"],
  ...
  },
  {
    ...
    body: "Controller の一種。戻り値を JSON などの本文にします。テンプレートは使いません。Spring のアノテーションです。",
  },
  {
    term: "@ResponseBody",
  ...
  },
  {
    term: "Service",
    aliases: ["Service"],
    body: "ビジネスロジックを置く層です。業務として何をするかを書きます。その操作を進めてよいかの判定、状態の更新、メールなどの通知が、ここにまとまることが多いです。Spring ではこの名前が多いです。",
  },
  {
    term: "Repository",
    aliases: ["Repository"],
  ...
  },
  {
    ...
    body: "Thymeleaf で、テンプレートの一部に名前を付け、ほかのテンプレートから差し込んで使う仕組みです。共通のヘッダや CSS の読み込みなど、複数画面で同じ HTML を 1 か所にまとめられます。名前を付ける側が `th:fragment`、差し込む側が `th:replace` です。",
  },
  {
    term: "JSP",
    aliases: ["JSP"],
  ...
  },
  {
    term: "N+1",
    aliases: ["N+1"],
    body: "一覧の件数と同じ回数だけ、追加の SQL が飛ぶパターン。画面は動きますが、遅くなります。",
  },
  {
    term: "Filter",
  ...
  },
  {
    term: "コミット",
    aliases: ["コミット"],
    body: "「コミット」は DB と Git のどちらの文脈でも使われます。DB では、トランザクションを確定する操作です（これが無いと更新が DB に残りません）。Git では、変更を 1 つの単位として記録する操作です（`git commit` やコミットハッシュなど）。",
  },
  {
    term: "ロールバック",
    aliases: ["ロールバック"],
  ...
  },
  {
    ...
    kana: "けつごう",
    aliases: ["結合", "JOIN"],
    body: "複数のテーブルを、共通のカラムで紐付けて 1 つの結果にまとめる SQL の操作です。SQL では JOIN と書きます。",
  },
  {
    term: "LEFT JOIN",
  ...
  },
  {
    term: "NAT",
    aliases: ["NAT"],
    body: "複数の端末が、外向きには 1 つ（または少数）の IP アドレスに見えるよう変換する仕組みです。社内ネットワークやルータでよく使われます。access.log の接続元 IP に、利用者本人ではなくこの NAT のアドレスが残ることがあります。",
  },
  {
    term: "リバースプロキシ",
    aliases: ["リバースプロキシ"],
  ...
  },
  {
    term: "セキュリティグループ",
    aliases: ["セキュリティグループ"],
    body: "クラウド（AWS など）でインスタンスやサービスに設定する、仮想的な通信制御です。許可した通信だけを通すのが基本で、オンプレの FW やルータの ACL と役割は近いですが、対象やかけ方が違います。",
  },
  {
    term: "ポリシーベースルーティング",
  ...
  {
    term: "パーミッション",
    aliases: ["パーミッション", "アクセス権限"],
    body: "ファイルやディレクトリに対する、読み取り・書き込み・実行の権限です。`ls -l` の先頭の `-rwxr-xr-x` のような文字列で見えます。所有者・グループ・それ以外で別々に設定されます。",
  },
  {
    term: "SELinux",
    aliases: ["SELinux"],
    body: "Linux のセキュリティ機能の 1 つです。`ls -l` で見える通常のパーミッションとは別の仕組みで、ファイルやプロセスにセキュリティコンテキストというラベルを付けて制御します。RHEL 系でよく有効です。通常のパーミッションが正しくても、SELinux で拒否されることがあります。",
  },
  {
    term: "プロセス",
    aliases: ["プロセス"],
    body: "実行中のプログラムの単位です。Java アプリなら、起動すると 1 つの Java プロセスとして動きます。`ps` コマンドで、起動しているプロセスの一覧を見られます。",
  },
  {
    term: "システムコール",
    aliases: ["システムコール", "syscall"],
    body: "ファイルを開く、ネットワークにつなぐなど、プロセスが OS に依頼する処理です。",
  },
  {
    term: "kill",
  ...
  {
    term: "ガベージコレクション",
    aliases: ["ガベージコレクション", "GC", "Full GC"],
    body: "もう使われなくなったオブジェクトを、JVM が自動でヒープから回収する仕組みです。回収中は処理が一時的に止まることがあります。`Full GC` はヒープ全体を対象にした回収で、時間がかかりやすいです。",
  },
  {
    term: "OutOfMemoryError",
    aliases: ["OutOfMemoryError", "OOM"],
    body: "JVM がメモリを確保できなくなったときに投げられる `Error`（`Exception` ではありません）です。`Java heap space` はヒープ不足、`Metaspace` はクラス情報の領域不足など、メッセージによって疑う場所が変わります。",
  },
  {
    term: "スレッドダンプ",
    aliases: ["スレッドダンプ", "thread dump"],
    body: "ある瞬間に、動いている各スレッドが何をしているかを書き出した記録です。`jstack` などで取得します。処理が返ってこないときに、どこで止まっているかを確認する手がかりになります。",
  },
  {
    term: "sudo",
    aliases: ["sudo"],
  ...
  },
  {
    ...
    body: "IDE の機能。今の Java メソッドを起点に、呼び出し元と呼び出し先を階層（ツリー）で出します。日本語化した IntelliJ では「呼び出し階層」、Eclipse では「呼び出し階層を開く」です。1 段だけなら参照検索と定義へジャンプで十分です。",
  },
  {
    term: "実装へジャンプ",
    kana: "じっそうへジャンプ",
  ...
  },
  {
    term: "git bisect",
    aliases: ["git bisect"],
    body: "正常だった時点と異常な時点を指定すると、Git が残りのコミットを自動で二分探索し、原因のコミットを絞り込んでくれる機能です。あいだのコミット数が多くて 1 件ずつ確認できないときに使います。",
  },
  {
    term: "SVN",
  ...
  },
  {
    term: "タイムアウト",
    aliases: ["タイムアウト"],
    body: "決められた時間を過ぎると、処理や接続を切ること。応答の無い接続を切る場合と、操作の無いセッションを切る場合があります。",
  },
  {
    term: "ロック",
    aliases: ["行ロック", "レコードロック", "ロック"],
    body: "同時に進めないよう、一時的に専有すること。待ちの原因になります。DB のレコードやテーブルのロックのほか、Java の `synchronized` などスレッドが持つロックもあります。",
  },
  {
    term: "分離レベル",
    kana: "ぶんりレベル",
  ...
  },
  {
    ...
    body: "分離レベルの 1 つです。同じトランザクションの中で同じ SELECT を繰り返すと、他のトランザクションが確定した変更を挟んでも、同じ結果が読めます。MySQL（InnoDB）の既定の分離レベルです。",
  },
  {
    term: "楽観ロック",
    kana: "らっかんロック",
  ...
  },
  {
    term: "デッドロック",
    aliases: ["デッドロック"],
    body: "複数の処理が、お互いが持つロックの解放を待ち合い、どちらも先に進めなくなることです。DB のレコードロックでも、Java のスレッドが持つロックでも起こります。ロックする順番をそろえると防げることが多いです。",
  },
  {
    term: "モック",
    aliases: ["モック"],
  ...
  },
  {
    term: "バンドル",
    aliases: ["バンドル", "bundle"],
    body: "複数の JavaScript ファイルを 1 つ（または少数）にまとめること。webpack や vite が行います。まとめる際に、変数名を短くする最適化（難読化）も一緒に行われることが多いです。",
  },
  {
    term: "ハッシュ",
    aliases: ["ハッシュ", "hash"],
  ...
  },
  {
    ...
    body: "JavaScript をビルドする代表的なツールの 1 つ。複数のファイルを 1 つにまとめ（バンドル）、変数名を短くする最適化（難読化）もします。",
  },
  {
    term: "vite",
    aliases: ["vite", "Vite"],
  ...
  },
  {
    term: "メソッド",
    aliases: ["メソッド"],
    body: "「メソッド」は HTTP と Java のどちらの文脈でも使われます。HTTP では、GET / POST / PUT などのやり取りの種類です（Network タブや URL の話で出てきます）。Java では、クラスの中の処理のまとまりです（`list()` などのメソッド名や、クラス名の話で出てきます）。",
  },
  {
    term: "オブジェクト",
    aliases: ["オブジェクト"],
  ...
  },
  {
    term: "GUI",
    aliases: ["GUI"],
    body: "画面上のボタンやメニューを操作する形式のことです。文字だけで操作するコマンド操作（CLI）と対になる言葉です。",
  },
  {
    term: "デバッガ",
    aliases: ["デバッガ"],
