...
const linuxLesson = { to: "/tracks/troubleshoot/linux-basics", label: "Linux の基本操作" };
const httpLogLesson = { to: "/tracks/troubleshoot/http-server-log", label: "HTTP サーバのログを見る" };
const middlewareLesson = { to: "/tracks/troubleshoot/middleware-check", label: "ミドルウェアとコンテナの確認" };
const recentChangeLesson = { to: "/tracks/troubleshoot/recent-change", label: "いつから起きているかを疑う" };
const netCheckLesson = { to: "/tracks/troubleshoot/net-check", label: "ネットワークの疎通確認" };
const externalLesson = { to: "/tracks/troubleshoot/p-external", label: "トラブル例：外部システム / 外部 API" };
const slowLesson = { to: "/tracks/troubleshoot/p-slow", label: "トラブル例：遅い" };
const memoryLesson = { to: "/tracks/troubleshoot/p-memory", label: "トラブル例：メモリ不足・GC の当たりをつける" };
const hangLesson = { to: "/tracks/troubleshoot/p-hang", label: "トラブル例：処理が返ってこない（スレッドダンプ）" };
const logFollowLesson = { to: "/tracks/troubleshoot/log-follow", label: "アプリのログで処理を追う" };
const ymlLesson = { to: "/tracks/java-map/yml", label: "application.yml / application.properties" };
    ...
    groups: [
      {
        ...
        rows: [
          { cmd: "`ssh ユーザ名@ホスト名`", env: "Linux", desc: "サーバのターミナルに入る", ...linuxLesson },
          { cmd: "`docker exec -it コンテナ名 bash`", env: "Docker", desc: "コンテナに入る", ...linuxLesson },
          { cmd: "`kubectl exec -it Pod名 -- bash`", env: "Kubernetes", desc: "Pod に入る", ...linuxLesson },
          { cmd: "`ls -l`", env: "Linux", desc: "ファイル一覧と、パーミッション・所有者を見る", ...linuxLesson },
        ],
      },
      {
        title: "ログを絞り込む・数える",
        ...
        rows: [
          { cmd: "`tail -f app.log | grep requestId=12`", env: "Linux", desc: "追記される行をリアルタイムで絞り込む", ...linuxLesson },
          ...
          { cmd: "`awk '$1 >= \"04:12:00.000\" && $1 <= \"04:13:00.000\"' app.log`", env: "Linux", desc: "行の先頭が時刻のログで、特定の時間帯だけを切り出す", ...logFollowLesson },
          {
      ...
      },
      {
        ...
        rows: [
          { cmd: "`awk '{print $9}' access.log | sort | uniq -c | sort -rn`", env: "Linux", desc: "ステータスコード別の件数を、多い順に集計する", ...httpLogLesson },
          { cmd: "`awk '$9 == 500 {print $7}' access.log | sort | uniq -c | sort -rn`", env: "Linux", desc: "500 が出ているパスだけを、多い順に集計する", ...httpLogLesson },
      ...
      },
      {
        title: "ポート・ファイル・ディスク",
        rows: [
          { cmd: "`lsof -i :8080`", env: "Linux", desc: "そのポートを使っているプロセスを見る", ...linuxLesson },
          { cmd: "`ss -ltnp | grep 8080`", env: "Linux", desc: "同じことを `ss` で見る。`lsof` が無い環境向け", ...linuxLesson },
          { cmd: "`lsof app.log`", env: "Linux", desc: "そのファイルを開いているプロセスを見る", ...linuxLesson },
          { cmd: "`df -h`", env: "Linux", desc: "ディスクの空き容量を見る", ...linuxLesson },
          { cmd: "`free -h`", env: "Linux", desc: "メモリの空き容量を見る", ...linuxLesson },
      ...
      },
      {
        ...
        rows: [
          { cmd: "`systemctl status nginx`", env: "Linux", desc: "サービスの稼働状態を見る", ...middlewareLesson },
          { cmd: "`systemctl list-unit-files --type=service | grep -i tomcat`", env: "Linux", desc: "正しいユニット名を探す。停止中のサービスも一覧に出る", ...middlewareLesson },
      ...
      },
      {
        ...
        rows: [
          ...
          { cmd: "`systemctl restart tomcat9`", env: "Linux", desc: "サービスを再起動する", ...middlewareLesson },
      ...
      },
      {
        title: "直近の変更を確認する",
        rows: [
          { cmd: "`stat application.yml`", env: "Linux", desc: "ファイルの Modify（中身の変更） / Change（メタデータの変更）時刻を見る", ...recentChangeLesson },
          { cmd: "`docker inspect --format='{{.Created}}' shinsei-app`", env: "Docker", desc: "コンテナの作成時刻を見る", ...recentChangeLesson },
          { cmd: "`find /opt/app -newer application.yml -type f`", env: "Linux", desc: "特定のファイルより後に更新された（＝配置された）ファイルだけを探す。デプロイで一括更新された範囲の当たりをつける", ...recentChangeLesson },
          { cmd: "`find . -newermt \"2026-08-30 09:00\" ! -newermt \"2026-08-30 10:00\"`", env: "Linux", desc: "指定した時間帯だけに更新されたファイルを探す（GNU find の `-newermt`。日時を直接指定できる）", ...recentChangeLesson },
        ],
      },
      {
        title: "設定ファイルを横断して探す",
        ...
        rows: [
          { cmd: "`grep -Rn \"context-path\" --include=\"*.yml\" .`", env: "Linux", desc: "カレントディレクトリ以下の `.yml` ファイルから、そのキーを持つ行をファイル名・行番号付きで探す", ...ymlLesson },
          { cmd: "`grep -Rln \"shinsei_dev\" .`", env: "Linux", desc: "その値（DB 名やホスト名など）を含むファイルの名前だけを一覧する（`-l` は行内容ではなくファイル名を出す指定）", ...ymlLesson },
    ...
    groups: [
      {
        title: "実行計画とレコードの確認",
        rows: [
          { cmd: "`EXPLAIN SELECT * FROM t_request WHERE applicant_id = 7;`", env: "MySQL", desc: "その SQL の実行計画（DB がどう読むか）を見る", ...slowLesson },
          { cmd: "`EXPLAIN ANALYZE SELECT * FROM t_request WHERE applicant_id = 7;`", env: "MySQL", desc: "実行計画に、実際にかかった時間も添えて見る（MySQL 8.0.18 以降）" },
          { cmd: "MyBatis の DEBUG ログ（`Preparing` / `Parameters` / `Total`）", env: "MyBatis", desc: "発行された SQL 文、バインド値、件数を見る", ...logFollowLesson },
          { cmd: "`SELECT id, title, status, created_at, updated_at FROM t_request WHERE id = 11;`", env: "MySQL", desc: "レコードの更新日時を直接確認する", ...recentChangeLesson },
        ],
      },
      {
    ...
    groups: [
      {
        ...
        rows: [
          { cmd: "`ping intranet.example.co.jp`", env: "Linux / Windows", desc: "ホストが応答するか（ICMP。HTTP とは別）", ...netCheckLesson },
          { cmd: "`Test-NetConnection -ComputerName intranet.example.co.jp -Port 8080`", env: "Windows", desc: "そのポートで TCP 接続できるか", ...netCheckLesson },
          { cmd: "`nc -zv intranet.example.co.jp 8080`", env: "Linux", desc: "同じ確認を `nc` で行う", ...netCheckLesson },
          { cmd: "`curl -I http://intranet.example.co.jp:8080/shinsei/requests`", env: "Linux / Windows", desc: "HTTP でパスまで届くか。ヘッダだけを見る", ...netCheckLesson },
          { cmd: "`curl -vk https://intranet.example.co.jp/shinsei/requests`", env: "Linux / Windows", desc: "TLS のハンドシェイクまで含めて詳しく見る", ...netCheckLesson },
          { cmd: "`traceroute -T -p 8080 intranet.example.co.jp`", env: "Linux", desc: "HTTP と同じ TCP で、途中どこまで届いているかを見る", ...netCheckLesson },
          { cmd: "`nslookup intranet.example.co.jp`", env: "Linux / Windows", desc: "名前解決できるか、どの IP を引いているかを見る", ...externalLesson },
        ],
      },
      {
        title: "応答内容・応答時間を掘る",
    ...
    groups: [
      {
        ...
        rows: [
          { cmd: "`jstack PID`", env: "JDK", desc: "その瞬間の全スレッドの状態を書き出す", ...hangLesson },
          { cmd: "`docker exec -it コンテナ名 jstack PID`", env: "Docker", desc: "Docker コンテナの中の Java プロセスに対して取る", ...hangLesson },
          { cmd: "`kubectl exec -it Pod名 -- jstack PID`", env: "Kubernetes", desc: "Kubernetes の Pod の中の Java プロセスに対して取る", ...hangLesson },
      ...
      },
      {
        title: "GC・メモリ",
        rows: [
          { cmd: "`jstat -gcutil PID 1000`", env: "JDK", desc: "1 秒おきに GC の状況を表示する。`FGC` は `Full GC` の回数、`FGCT` はその合計時間", ...memoryLesson },
          { cmd: "`jcmd PID GC.heap_info`", env: "JDK", desc: "今のヒープの使用状況を表示する", ...memoryLesson },
          { cmd: "`jmap -dump:format=b,file=heap.hprof PID`", env: "JDK", desc: "ヒープの中身をファイルに書き出す（ヒープダンプ）", ...memoryLesson },
