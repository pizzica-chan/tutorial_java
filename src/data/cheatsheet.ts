      ...
      },
      {
        ...
        rows: [
          ...
          { cmd: "`awk '$1 >= \"04:12:00.000\" && $1 <= \"04:13:00.000\"' app.log`", env: "Linux", desc: "行の先頭が時刻のログで、特定の時間帯だけを切り出す" },
          {
            ...
            env: "Linux",
            desc: "`04:12:00` を含む最初の行から、次に `04:13:00` を含む行までを、まるごと抜き出す。時刻に限らず、2 つの目印文字列で挟まれた範囲を取り出したいときにも使える",
          },
        ],
      },
      {
  ...
  },
  {
    ...
    groups: [
      ...
      },
      {
        ...
        note: "いつから起きているかは分かっても、その間に何十件もコミットがあって 1 件ずつ追えないときの手段です。",
        rows: [
          {
    ...
    groups: [
      {
        ...
        rows: [
          ...
          { cmd: "`curl -I http://intranet.example.co.jp:ポート番号/パス`", env: "Linux / Windows", desc: "HTTP でパスまで届くか。ヘッダだけを見る" },
          { cmd: "`curl -vk https://intranet.example.co.jp/パス`", env: "Linux / Windows", desc: "TLS のハンドシェイクまで含めて詳しく見る" },
    ...
    groups: [
      {
        ...
        note: "1 回だけでなく、数秒おきに 2〜3 回取って比べましょう。同じスレッドが毎回同じ場所で止まっていれば、そこが疑わしい箇所です。`jstack`/`jcmd` はどちらも JDK 同梱のツールで、JRE だけの軽量コンテナには入っていないことがあります（申請くんの Docker イメージもこの構成）。その場合は `kill -3` を使いましょう。",
        rows: [
