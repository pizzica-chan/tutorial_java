  ...
  lessons: [
    {
      ...
      title: "リクエストとレスポンス",
      minutes: 8,
      summary: "画面は結果。本体は HTTP の往復です。",
      blocks: [
        {
          type: "p",
          text: "ブラウザはサーバへリクエストを送り、サーバはレスポンスを返します。画面に見える HTML は、その応答の一種です。",
        },
        { type: "diagram", name: "http-roundtrip", caption: "画面は結果。本体はこの往復です。" },
        {
          type: "ul",
          items: [
    ...
    },
    {
      ...
      minutes: 9,
      summary: "画面側とサーバ側。データの実体は DB。",
      blocks: [
        {
          type: "p",
          text: "Web アプリは、動く場所で二つに分けて見ます。フロントエンドはブラウザ側、バックエンドはサーバ側です。境目は HTTP の往復です。一覧の件数や申請の中身の実体は、その奥の DB にあります。",
        },
        { type: "diagram", name: "front-back", caption: "フロントとバックの境目は HTTP。件数や更新結果の実体は DB です。" },
        {
          type: "table",
          ...
          rows: [
            ...
            ["見るもの", "画面、開発者ツール、Network", "ソース、ログ、スタックトレース", "件数、中身、承認者などの実データ"],
          ],
        },
        {
          type: "p",
        ...
        {
          type: "p",
          text: "フロントは返ってきた結果を出します。バックエンドは SQL で DB を読み書きします。コードが正しくても、検証 DB に行が無い、マスタが違う、別の DB を見ている、と画面は空や古い値になります。",
        },
        {
          type: "ul",
          items: [
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "p",
          text: "GET は取得が基本で、再読込しても副作用が小さいことが期待されます。POST は登録・更新・削除など、状態を変える操作に使います。画面のフォームでも、JSON を返す Web API でも、この意味は同じです。API では PUT / PATCH / DELETE もよく使います。",
        },
