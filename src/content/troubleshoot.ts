...
java    1842 appuser   8w   REG    8,1    48213 123457 app.log`,
        ...
        {
          type: "p",
          text: "ここで見たのは最低限です。ログの絞り込みやパケットキャプチャなど、組み合わせた実務向けのコマンドはチートシートにまとめています。",
          link: {
            label: "チートシート",
            to: "/cheatsheet",
          },
...
t_request  ref   fk_request_applicant   fk_request_applicant   3`,
    ...
    at jp.co.example.shinsei.service.RequestService.searchHistory(RequestService.java:66)`,
        ...
        {
          type: "p",
          text: "`->` の前後がヒープの使用量（回収前 → 回収後）で、括弧の中はそのときのコミット済みヒープサイズ（今 OS から確保している大きさ）です。ヒープは `-Xms` から `-Xmx` の範囲で伸び縮みするため、常に `-Xmx` の設定と一致するとは限りません。回収してもほとんど減っていなければ、解放できるはずのオブジェクトが残っている、つまりメモリリークの兆候です。",
        },
        {
          type: "h2",
          text: "簡単に確認できるコマンド",
