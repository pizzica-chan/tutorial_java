...
Change: 2026-08-30 09:58:11.000000000 +0900`,
        ...
        {
          type: "p",
          text: "ここで主に見るのは Change です。コピーや配置そのものでも必ず更新されるので、`rsync -a` や `cp -p` のように元のタイムスタンプを保つデプロイでも、このサーバに実際に配置された時刻を反映します。Change が症状の報告時刻の直前にあれば、その配置が原因の候補です。Modify は中身がいつ書かれたか（開発時の編集時刻など）の参考程度に留めましょう。",
        },
        {
          type: "callout",
          kind: "note",
...
2026-08-30T00:58:22.104512Z  # UTC。日本時間では 09:58:22`,
        ...
        {
          type: "p",
          text: "レコードに更新日時のカラムがあれば、それも手がかりになります。ただし、そのカラムが実際にきちんと更新されているかは、アプリや DB の設計次第です。更新のたびに書き込まれているとは限りません。",
        },
        {
          type: "code",
