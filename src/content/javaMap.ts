    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          ...
          rows: [
            ["1: 内蔵だけ", "手前の HTTP サーバが無いので、パスのずれやコンテキストパスの重複は起きません。アプリのログも自分の標準出力そのものです。"],
            ["2: 外部 WAR", "手前の HTTP サーバは無いので、パスのずれやコンテキストパスの重複は起きません。アプリのログは、Tomcat 側の catalina.out などに出ることがあります。"],
            ["3: 手前に Apache / nginx", "静的ファイルの 404 は、手前のパス設定のことがあります。アプリのエラーログは、手前の error.log ではなくサーブレットコンテナ側（外部 Tomcat なら catalina.out など）です。コンテキストパスが、手前と後ろの両方に付いていることもあります。"],
          ],
        },
        {
          type: "h2",
