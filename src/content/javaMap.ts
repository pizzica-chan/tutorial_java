...
// 本体の1行目で止まった時点では、トランザクションが始まっていることがある`,
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "p",
          text: "気をつけることは、上のどのパターンかで変わります。ただし、ログの出力先自体は logback などの設定次第です。ここでは、コンソールにそのまま出す設定のときの既定の動きを整理します。手前に Apache / nginx がある構成（パターン3）でだけ増える確認が多いです。",
        },
        {
          type: "table",
          headers: ["パターン", "気をつけること"],
          rows: [
            ["1: 内蔵だけ", "手前の HTTP サーバが無いので、パスのずれやコンテキストパスの重複は起きません。ログをコンソールに出す設定なら、起動したプロセスの標準出力にそのまま出ます。"],
            ["2: 外部 WAR", "手前の HTTP サーバは無いので、パスのずれやコンテキストパスの重複は起きません。ログをコンソールに出す設定なら、Tomcat 側の catalina.out などに出ることがあります。"],
            ["3: 手前に Apache / nginx", "静的ファイルの 404 は、手前のパス設定のことがあります。アプリのエラーログは、手前の error.log ではなくサーブレットコンテナ側（ログをコンソールに出す設定で、外部 Tomcat なら catalina.out など）です。コンテキストパスが、手前と後ろの両方に付いていることもあります。"],
          ],
        },
        {
          type: "h2",
