...
------------------------------------------------------------------`,
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "p",
          text: "代入した箇所と、それが原因で例外になる箇所が、別のタイミングで動く別のコードのこともあります。たとえば、申請の登録時に承認者が未選択のまま登録されると、承認者の ID は `null` のまま保存されます。この値を後日の承認処理で使おうとすると、NPE になります。",
        },
        {
          type: "code",
...
if (!"PENDING".equals(request.getStatus())) {
  ...
  return "redirect:/requests/" + id;
        ...
        },
        {
          type: "ul",
          items: [
            "止まらないときは、その行に処理が来ていないとは限らない。デバッグ実行になっているか、Filter や別メソッド、別プロセスで処理されていないかを疑う",
