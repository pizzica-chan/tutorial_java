  ...
  lessons: [
    {
      ...
      blocks: [
        {
          type: "p",
          text: "この章では、ソースの読み方をいくつかの手法に分けています。どれを先に使うかは、いま分かっている手がかりで決まります。",
        },
        {
          type: "table",
          ...
          rows: [
            ...
            [
              "探したい SQL が先にある。テーブル名や文の一部から発行元のソースを知りたい",
              "次章「SQL からソースを探す」",
            ],
          ],
        },
      ...
      blocks: [
        ...
        },
        {
          ...
          items: [
            ...
            { title: "永続化・外部呼び出し", text: "DB への SQL 実行やファイルの書き込みなど永続化のほか、外部 API の呼び出しもここに含みます。" },
  ...
  int update(RequestEntity request);
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "p",
          text: "実行された SQL からソースを探す手順は、次章「SQL からソースを探す」にあります。MyBatis と JPA では探し方が違います。",
        },
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "p",
          text: "そのまま探す・数字 — ドットや空白を特別扱いしないで探すときに使います。",
        },
        {
          type: "table",
          headers: ["書き方", "意味", "ヒットする例"],
          rows: [
            [
              "`application\\.yml`",
              "`\\.` でドットをそのまま",
              "「`application.yml`」— ヒットする。escape せず `.` だけで探すと、「`application-yml`」のような文字列にも誤ってヒットしてしまう",
            ],
            [
              "`\\s`",
              "空白1つ分",
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "p",
          text: "承認ボタンを押すと `POST /shinsei/requests/12/approve` が飛びます。処理の入口は `RequestController.approve` で、そこから `RequestService.approve` へ降ります。",
        },
        {
          type: "code",
  ...
  public String approve(
    ...
    return "redirect:/requests";
        ...
        },
        {
          ...
          text: "定義へジャンプは、Java の宣言に着きます。RequestMapper はインタフェースなので、着くのはメソッドの宣言です。SQL の文は XML 側です。手順は次章「SQL からソースを探す」です。",
        },
        {
          type: "h2",
          text: "呼び出し元を一覧する",
...
requestService.approve(id, user.getId());`,
        ...
        {
          type: "p",
          text: "一覧にバッチやテストが出ることがあります。メソッドの中身は正しく見えても結果が違うときは、今調べている処理の入口とは別の呼び出し元を疑いましょう。画面用とバッチ用で実装が二つ、など。",
        },
        {
          type: "h2",
          text: "呼び出しを階層で見る",
        ...
        {
          type: "p",
          text: "呼び出し元や呼び出し先が複数あるときは、呼び出し階層が使えます。今のメソッドを起点に、呼び出し元と呼び出し先が、まとめて階層（ツリー）になります。1段だけなら、参照検索と定義へジャンプで足ります。",
        },
        {
          type: "table",
        ...
        {
          type: "p",
          text: "メソッド本体だけを読むと見落とすことがあります。辿れるようになったら、中身の前に次を見ましょう。",
        },
        {
          type: "ul",
          items: [
        ...
        {
          type: "p",
          text: "最後の「同じ型の別実装が無いか」を確認する操作が、実装へジャンプです。同じインタフェースの実体クラスを開きます。モックや、プロファイルで切り替わる実装を見るときに使います。",
        },
        {
          type: "ul",
          items: [
...
1187   sato-t     throw new ConflictException("この申請は承認できません");`,
        ...
        {
          type: "p",
          text: "`-r 1200` は、リビジョン 1200 の時点のファイルを見るという指定です。行頭の 1187 は、その行を最後に変更したリビジョン番号なので、指定した番号とは限りません。",
        },
        {
          type: "code",
          title: "svn log -r（例）",
...
------------------------------------------------------------------`,
    ...
    },
    {
      ...
      blocks: [
        {
          type: "p",
          text: "`NullPointerException` や表示不正の多くは、問題になっている変数がセットされた場所に原因があります。",
        },
  ...
  VALUES (#{title}, #{status}, #{applicantId}, #{approverId},
    ...
    },
    {
      ...
      blocks: [
        ...
        {
          type: "p",
          text: "見るだけでなく、書き換えることもできます。特定の条件でしか通らない `if` のように、その状況を毎回用意するのが大変な分岐でも、止めた場所で変数を狙った値に書き換えれば、実際にその分岐を通して確認できます。",
        },
        {
          type: "p",
...
<script th:src="@{/js/form.js}"></script>`,
        ...
        {
          type: "p",
          text: "手がかりになるクラス名も `id` も無いときや、検索してもヒットしないときは、デバッガで止めて確かめましょう。",
        },
        {
          type: "ol",
          items: [
    ...
    },
    {
      ...
      blocks: [
        ...
        },
        {
          type: "ol",
          items: [
            "求められているのが「文書どおりの動作」か「今の動きの不具合修正」かを、依頼者に確認する",
            "コードと、検証用環境の実挙動を見る",
            "どちらを正とするか、依頼者や仕様書の作成者など、判断できる人に確認する",
            "影響範囲を見ずに仕様を書き換えない",
          ],
        },
      ],
