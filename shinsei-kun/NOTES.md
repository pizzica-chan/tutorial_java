...
- **申請履歴のステータス検索は、条件に乗らない。** フォームの name は `status`、Controller の `@RequestParam` は `requestStatus` です。シナリオ「申請履歴検索の結果が不正」用です。件名と申請日は効きます。識別子を揃えて直してはいけません。
...
- **初期データは 5 件。** 教材の一覧 HTML 例は「交通費申請」「備品購入」の 2 行です。実データは休暇申請・出張旅費・研修参加が足されています。`ORDER BY created_at DESC` なので、交通費申請（ID 12）は先頭ではありません。
