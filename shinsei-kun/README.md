...
山田でログインすると、承認ボタンは一覧にも詳細にも見えます。一覧で押しても、教材シナリオのため POST しません（`static/js/list.js`）。詳細で押すと POST します。山田は承認者ではないので「権限がありません」になります。佐藤が詳細で承認すると通ります。確認ダイアログは `static/js/app.js` です。
